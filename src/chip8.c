...
void initializeScreen() {
  ...
  if ((screen.window = SDL_CreateWindow(
           "Chip8", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
           DISPLAY_WIDTH * RENDER_SCALE, DISPLAY_HEIGHT * RENDER_SCALE, 0)) ==
      NULL) {
    fprintf(stderr, SDL_GetError());
    exit(1);
  }
  ...
  if (SDL_RenderSetLogicalSize(screen.renderer, DISPLAY_WIDTH,
                               DISPLAY_HEIGHT) != 0) {
    fprintf(stderr, SDL_GetError());
    exit(1);
  }
...
void printRegisters() {
  ...
  printf("v[0]: 0x%x v[1]: 0x%x v[2]: 0x%x v[3]: 0x%x\nv[4]: 0x%x v[5]: 0x%x "
         "v[6]: 0x%x v[7]: 0x%x\nv[8]: 0x%x v[9]: 0x%x v[A]: 0x%x v[B]: "
         "0x%x\nv[C]: 0x%x v[D]: 0x%x v[E]: 0x%x v[F]: 0x%x\n",
         v[0], v[1], v[2], v[3], v[4], v[5], v[6], v[7], v[8], v[9], v[0xA],
         v[0xB], v[0xC], v[0xD], v[0xE], v[0xF]);
  ...
  while (is_running) {
    ...
    case 0x8:
      ...
      case 0x0:
        ...
        break;
      case 0x1:
        printf("Set v[%d] to v[%d] | v[%d]", second_nibble, second_nibble,
               third_nibble);
      ...
      case 0x3:
        ...
        break;
      case 0x4:
        printf("Set v[%d] = v[%d] + v[%d]", second_nibble, second_nibble,
               third_nibble);
      ...
      case 0x6:
        ...
        break;
      case 0x7:
        printf("Set v[%d] = v[%d] - v[%d]", second_nibble, third_nibble,
               second_nibble);
    ...
    case 0xD: // Draw case
      ...
      for (int draw_row = 0; draw_row < fourth_nibble; draw_row++) {
        ...
        for (int draw_pixel_col = 0;
             draw_pixel_col < 8 && draw_x + draw_pixel_col < DISPLAY_WIDTH;
             draw_pixel_col++) {
    ...
    for (int x_pixel = 0; x_pixel < DISPLAY_WIDTH; x_pixel++) {
      for (int y_pixel = 0; y_pixel < DISPLAY_HEIGHT; y_pixel++) {
        if (display[x_pixel][y_pixel]) {
          SDL_SetRenderDrawColor(screen.renderer, 255, 255, 255,
                                 SDL_ALPHA_OPAQUE);
